...
export const Route = createFileRoute('/_public/politica-de-reembolso')({
...
function RefundPolicyPage() {
  ...
  const { data: page, isLoading, isError } = usePublicPage('politica-de-reembolso')

  return (
    <>
      <Helmet>
        <title>{page?.title ?? 'Política de Reembolso'} — {siteName}</title>
        <meta
          name="description"
          content="Consulte as regras de devolução e reembolso aplicáveis às encomendas da Cheesemania."
        />
      </Helmet>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <header className="mb-7 md:mb-10">
            ...
            <h1 className="font-sans font-black text-stone-900 text-3xl md:text-4xl leading-tight">
              {page?.title ?? 'Política de Reembolso'}
            </h1>
          </header>
          ...
          <article className="bg-white/95 backdrop-blur rounded-3xl border border-primary-100 shadow-[0_22px_60px_-40px_rgba(17,24,39,0.45)] px-6 py-7 md:px-9 md:py-10">
            ...
            {(isError || !page) && !isLoading && (
              <p className="text-stone-500">A política de reembolso ainda não está disponível.</p>
            )}
