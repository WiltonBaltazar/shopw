...
function PrivacyPolicyPage() {
  ...
  const { data: page, isLoading, isError } = usePublicPage('politica-de-privacidade')

  return (
    <>
      <Helmet>
        <title>{page?.title ?? 'Política de Privacidade'} — {siteName}</title>
        <meta
          name="description"
          content="Leia a nossa política de privacidade e saiba como protegemos os seus dados pessoais."
        />
      </Helmet>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <article className="bg-white/95 backdrop-blur rounded-3xl border border-primary-100 shadow-[0_22px_60px_-40px_rgba(17,24,39,0.45)] px-6 py-7 md:px-9 md:py-10">
            ...
            {(isError || !page) && !isLoading && (
              <p className="text-stone-500">A política de privacidade ainda não está disponível.</p>
            )}
